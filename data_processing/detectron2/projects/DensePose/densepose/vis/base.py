...
class MatrixVisualizer(object):
    ...
    def visualize(self, image_bgr, mask, matrix, bbox_xywh):
        ...
        # matrix_scaled_8u = matrix_scaled.clip(0, 255).astype(np.uint8)
        # matrix_vis = cv2.applyColorMap(matrix_scaled_8u, self.cmap)
        # print('matrix', matrix.min(), matrix.max(),matrix.shape)
        matrix = matrix.cpu().numpy()
        matrix_vis = np.zeros((matrix.shape[0], matrix.shape[1], 3), dtype=np.uint8)
        matrix_vis[:,:,0] = matrix//255
        matrix_vis[:,:,1] = matrix%255




        matrix_vis[mask_bg] = image_target_bgr[y: y + h, x: x + w, :][mask_bg]
        image_target_bgr[y: y + h, x: x + w, :] = matrix_vis
