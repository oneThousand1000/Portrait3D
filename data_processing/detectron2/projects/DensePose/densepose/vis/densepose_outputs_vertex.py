...
class DensePoseOutputsVertexVisualizer(object):
    ...
    def visualize(
        ...
        for n in range(N):
            ...
            #print('closest_vertices: ', closest_vertices.shape,closest_vertices.max(),closest_vertices.min())
            # embed_map = get_xyz_vertex_embedding(mesh_name, self.device)
            # vis = (embed_map[closest_vertices].clip(0, 1) * 255.0).cpu().numpy()
            # print('vis: ', vis.shape, vis.max(), vis.min())



            mask_numpy = mask.cpu().numpy().astype(dtype=np.uint8)
            image_bgrs.append(self.mask_visualizer.visualize(image_bgr.copy(), mask_numpy, closest_vertices, [x, y, w, h]))
