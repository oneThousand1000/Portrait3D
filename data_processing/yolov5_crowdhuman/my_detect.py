...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for idx in range(len(coco_joint_list)):
            ...
            # sort face points
            face_points = face_points[np.argsort(face_points[:, 2])]
            #print('face_points', face_points.shape)
            if np.sum(face_points[:, 2]) < 0.5:
                face_points = face_points[-2:,:]
            face_center = np.mean(face_points, axis=0, keepdims=True)
            for bbox in bbox_list_wo_sort:
                relax = 0.1
                relaxed_bbox = [bbox[0]-bbox[2]*relax, bbox[1]-bbox[3]*relax, bbox[2]*(1+2*relax), bbox[3]*(1+2*relax)]
                check = True
                for points_idx in range(face_points.shape[0]):
                    if not (relaxed_bbox[0] <= face_points[points_idx][0] <= relaxed_bbox[0]+relaxed_bbox[2] and relaxed_bbox[1] <= face_points[points_idx][1] <= relaxed_bbox[1]+relaxed_bbox[3]):
                        check = False
                        break
                if check:
                    bbox_list_sort.append(bbox)
                    break
            if len(bbox_list_sort) != idx+1:
                head_stride = max(np.max(face_points[:, 0]) - np.min(face_points[:, 0]),
                                  np.max(face_points[:, 1]) - np.min(face_points[:, 1])) * 1.2
                temp_bbox = [face_center[0][0]-head_stride/2, face_center[0][1]-head_stride/2, head_stride, head_stride]
                bbox_list_sort.append(temp_bbox)
        ...
        # save bbox
    with open(os.path.join(opt.source, 'head_bbox_yolov5_crowdhuman.json'), 'w') as f:
