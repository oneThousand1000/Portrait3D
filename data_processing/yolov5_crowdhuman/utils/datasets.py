...
class LoadImages:  # for inference
    def __init__(self, path, bbox_results, img_size=640, stride=32):
        p = str(Path(path).absolute())  # os-agnostic absolute path
        if '*' in p:
            files_ = sorted(glob.glob(p, recursive=True))  # glob
        elif os.path.isdir(p):
            files_ = sorted(glob.glob(os.path.join(p, '*.*')))  # dir
        ...
        files = []
        for f in files_:
            if os.path.basename(f) in bbox_results:
                continue
            else:
                files.append(f)
