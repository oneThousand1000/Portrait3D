...
from nerf.trigrid_utils import *
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_interval', type=int, default=5, help="test on the test set every interval epochs")
    ...
    ## Trigrid options
    parser.add_argument('--trigrid_path', type=str, default='', help="path to trigrid")
    parser.add_argument('--trigrid_decoder_ckpt', type=str, default='', help="path to trigrid decoder ckpt")
    parser.add_argument('--train_decoder', action='store_true', help="train trigrid decoder")
    parser.add_argument('--learnable_bg', action='store_true', help="Learnable background")
    parser.add_argument('--noise_bg', action='store_true', help="use noise background")
    ...
    # model options
    parser.add_argument('--bg_radius', type=float, default=3.0,
    ...
    # network backbone
    parser.add_argument('--backbone', type=str, default='trigrid', choices=['trigrid'], help="nerf backbone")
    ...
    parser.add_argument('--radius_range', type=float, nargs='*', default=[2.7, 2.71],
                        help="training camera radius range")
    parser.add_argument('--theta_range', type=float, nargs='*', default=[60, 105],
    ...
    parser.add_argument('--fovy_range', type=float, nargs='*', default=[11.5, 21], help="training camera fovy range")

    parser.add_argument('--default_radius', type=float, default=2.7, help="radius for the default view")
    ...
    parser.add_argument('--default_fovy', type=float, default=12., help="fovy for the default view")
    ...
    parser.add_argument('--zero123_ckpt', type=str, default='pretrained/zero123/zero123-xl.ckpt',
                        help="ckpt for zero123")
    ...
    if opt.learnable_bg:
        assert opt.bg_radius > max(
            opt.radius_range), f"bg_radius must be larger than max(radius_range) = {max(opt.radius_range)}"
        assert opt.noise_bg is False

    if opt.noise_bg:
        assert opt.learnable_bg is False

    assert opt.latent_iter_ratio == 1.0, "latent_iter_ratio must be 1.0 for now"
    ...
    from nerf.network_trigrid import NeRFNetwork
    from nerf.network_trigrid_latent import LatentNeRFNetwork

    model = LatentNeRFNetwork(
        opt=opt,
        device=device
    )
    teacher_model = NeRFNetwork(
        opt=opt,
        device=device
    )
    # load
    print(f'loading trigrid_renderer from {opt.trigrid_decoder_ckpt}...')
    ckpt = torch.load(opt.trigrid_decoder_ckpt, map_location=lambda storage, loc: storage)
    # ckpt = {'params': params, 'state_dict': ckpt}
    state_dict = ckpt['state_dict']
    state_dict_wo_torgb = {}
    for k, v in state_dict.items():
        if 'torgb' not in k:
            state_dict_wo_torgb[k] = v

    model.model.load_state_dict(state_dict_wo_torgb, strict=False)
    teacher_model.model.load_state_dict(state_dict)
    #
    model.model.rendering_kwargs['depth_resolution'] = 48
    model.model.rendering_kwargs['depth_resolution_importance'] = 48
    model.model.rendering_kwargs['ray_start'] = 2.35

    teacher_model.model.rendering_kwargs['depth_resolution'] = 48
    teacher_model.model.rendering_kwargs['depth_resolution_importance'] = 48
    teacher_model.model.rendering_kwargs['ray_start'] = 2.35
    #
    # load plane from pkl
    print(f'loading trigrid from {opt.trigrid_path}...')
    import pickle

    with open(opt.trigrid_path, 'rb') as f:
        data = pickle.load(f)
        trigrid = data['trigrids']
        ws = data['ws']
    ...
    # print(f'loading encoder from {opt.encoder_ckpt}...')
    # encoder_ckpt = torch.load('./pretrained/encoder_sd1.5.pt', map_location=lambda storage, loc: storage)
    # model.latent_net.load_state_dict(encoder_ckpt, strict=False)

    print('save trigrid to workspace...')
    shutil.copy(opt.trigrid_path, os.path.join(opt.workspace, 'trigrid.pkl'))

    train_loader = NeRFDataset(opt, device=device, type='train', H=opt.h, W=opt.w,
                               size=opt.dataset_size_train * opt.batch_size, teacher_H=opt.h * 8,
                               teacher_W=opt.w * 8).dataloader()

    if opt.optim == 'adan':
        from optimizer import Adan

        # Adan usually requires a larger LR
        optimizer = lambda model: Adan(model.get_params(5 * opt.lr), eps=1e-8,
                                       weight_decay=2e-5, max_grad_norm=5.0,
                                       foreach=False)
    else:  # adam
        optimizer = lambda model: torch.optim.Adam(model.get_params(opt.lr),
    ...
    trainer = TrigridTrainer(' '.join(sys.argv), 'trigrid', opt, model, teacher_model, guidance, device=device,
    ...
    valid_loader = NeRFDataset(opt, device=device, type='val', H=opt.H, W=opt.W,
                               size=opt.dataset_size_valid, teacher_H=opt.h * 8, teacher_W=opt.w * 8).dataloader(
        batch_size=1)
    test_loader = NeRFDataset(opt, device=device, type='test', H=opt.H, W=opt.W, size=opt.dataset_size_test,
                              teacher_H=opt.h * 8, teacher_W=opt.w * 8).dataloader(
        batch_size=1)

    # # test output
    # trainer.test(test_loader, save_path=os.path.join(opt.workspace, 'latent_trigrid_fit_initiation'))
