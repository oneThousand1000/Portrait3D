...
def rand_poses(size, device, opt, radius_range=[1, 1.5], theta_range=[0, 120], phi_range=[0, 360], return_dirs=False, angle_overhead=30, angle_front=60, uniform_sphere_rate=0.5,pivot = None):
...
def circle_poses(device, radius=torch.tensor([3.2]), theta=torch.tensor([60]), phi=torch.tensor([0]), return_dirs=False, angle_overhead=30, angle_front=60,pivot= None):
    assert pivot is not None
...
class NeRFDataset:
    def __init__(self, opt, device, type='train', H=256, W=256, size=100):
        ...
        self.cam_pivot = torch.tensor([0, 0.0649, 0], device=device).view(1, 3)
    ...
    def get_default_view_data(self):
        ...
        radii = torch.FloatTensor([self.opt.default_radius]).to(self.device)
        thetas = torch.FloatTensor(self.opt.ref_polars).to(self.device)
        phis = torch.FloatTensor(self.opt.ref_azimuths).to(self.device)


        poses, dirs = circle_poses(self.device, radius=radii, theta=thetas, phi=phis, return_dirs=True, angle_overhead=self.opt.angle_overhead, angle_front=self.opt.angle_front, pivot = self.cam_pivot)
        fov = self.opt.default_fovy
        focal = H * float(1 / (np.tan(fov * 3.14159 / 360) * 1.414)) # H / (2 * np.tan(np.deg2rad(fov) / 2))
    ...
    def collate(self, index):
        ...
        if self.training:
            ...
            poses, dirs, thetas, phis, radius = rand_poses(B, self.device, self.opt, radius_range=self.opt.radius_range, theta_range=self.opt.theta_range,
                                                           phi_range=self.opt.phi_range, return_dirs=True, angle_overhead=self.opt.angle_overhead, angle_front=self.opt.angle_front,
                                                           uniform_sphere_rate=self.opt.uniform_sphere_rate, pivot = self.cam_pivot)
            ...
            poses, dirs = circle_poses(self.device, radius=radius, theta=thetas, phi=phis, return_dirs=True, angle_overhead=self.opt.angle_overhead, angle_front=self.opt.angle_front, pivot = self.cam_pivot)
        ...
        focal = self.H * float(1 / (np.tan(fov * 3.14159 / 360) * 1.414)) #self.H / (2 * np.tan(np.deg2rad(fov) / 2))
