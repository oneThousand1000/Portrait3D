...
class NeRFDataset:
    def __init__(self, opt, device, type='train', H=256, W=256, size=100, teacher_H = None, teacher_W = None):
        super().__init__()
    ...
    def get_default_view_data(self):
        ...
        if self.teacher_W is not None:
            teacher_H =int(self.opt.known_view_scale * self.teacher_H)
            teacher_W = int(self.opt.known_view_scale * self.teacher_W)

            teacher_cx = teacher_H / 2
            teacher_cy = teacher_W / 2

            teacher_focal = teacher_H / (2 * np.tan(np.deg2rad(fov) / 2))
            teacher_intrinsics = np.array([teacher_focal, teacher_focal, teacher_cx, teacher_cy])

            teacher_rays = get_rays(poses, teacher_intrinsics, teacher_H, teacher_W, -1)
        ...
        data = {
            ...
            'teacher_H': self.teacher_W if self.teacher_W is not None else None,
            'teacher_W': self.teacher_W if self.teacher_W is not None else None,
            'teacher_rays_o': teacher_rays['rays_o'] if self.teacher_W is not None else None,
            'teacher_rays_d': teacher_rays['rays_d'] if self.teacher_W is not None else None,
        }
    ...
    def collate(self, index):
        ...
        if self.teacher_W is not None:
            teacher_H = self.teacher_H
            teacher_W = self.teacher_W
        ...
        data = {
            ...
            'teacher_H': teacher_H if self.teacher_W is not None else None,
            'teacher_W': teacher_W if self.teacher_W is not None else None,
            'teacher_rays_o': teacher_rays['rays_o'] if self.teacher_W is not None else None,
            'teacher_rays_d': teacher_rays['rays_d'] if self.teacher_W is not None else None,
        }
