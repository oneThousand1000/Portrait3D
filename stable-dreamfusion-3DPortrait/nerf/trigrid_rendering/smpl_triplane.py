...
class TriPlaneGenerator(torch.nn.Module):
    def __init__(self,
        ...
        self.bcg_synthesis = SynthesisNetwork(w_dim, img_resolution=self.superresolution.input_resolution,
                                              img_channels=self.decode_channel,
                                              **bcg_synthesis_kwargs) if rendering_kwargs.get('use_background',
                                                                                              False) else None
        ...
        planes = self.backbone.synthesis(torch.zeros(4, self.backbone.synthesis.num_ws, w_dim), update_emas=False,
                                         **synthesis_kwargs)
    ...
    def synthesis(self, ws, c, neural_rendering_resolution=None, update_emas=False, cache_backbone=False,
        ...
        render_output = self.renderer(planes, self.decoder, ray_origins,
                                      ray_directions, self.rendering_kwargs, apply_def=apply_def, ws=ws,
                                      pose_params=pose_params)  # channels last
        ...
        return {'image': sr_image, 'image_raw': rgb_image, 'image_depth': depth_image, "image_mask": mask_image,
                "image_background": bcg_rgb_image}
    ...
    def sample(self, coordinates, directions, z, c, p, truncation_psi=1, truncation_cutoff=None, update_emas=False,
               **synthesis_kwargs):
        # Compute RGB features, density for arbitrary 3D coordinates. Mostly used for extracting shapes.
        ws = self.mapping(z, c, p, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff,
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, neural_rendering_resolution=None,
        ...
        ws = self.mapping(z, c, pose_params, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff,
                          update_emas=update_emas)
        # TODO
