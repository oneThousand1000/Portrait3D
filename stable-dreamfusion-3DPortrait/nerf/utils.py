...
class Trainer(object):
        ...
        if self.opt.text is not None:
            ...
            if 'SD' in self.guidance:
                ...
                for d in ['front', 'side', 'back']:
                    self.embeddings['SD'][d] = self.guidance['SD'].get_text_embeds([f"{d} view {self.opt.text}"])

                    print('direction: ', d, 'prompt: ', f"{d} view {self.opt.text}")
            ...
            if 'IF' in self.guidance:
                ...
                for d in ['front', 'side', 'back']:
                    self.embeddings['IF'][d] = self.guidance['IF'].get_text_embeds([f"{d} view {self.opt.text}"])
    ...
    def train_step(self, data, save_guidance_path:Path=None):
        ...
        outputs = self.model.render(rays_o, rays_d, poses, H, W, staged=False, perturb=True, bg_color=bg_color, ambient_ratio=ambient_ratio, shading=shading, binarize=binarize)
    ...
    def eval_step(self, data):
        ...
        outputs = self.model.render(rays_o, rays_d, poses, H, W, staged=True, perturb=False, bg_color=None, light_d=light_d, ambient_ratio=ambient_ratio, shading=shading)
    ...
    def test_step(self, data, bg_color=None, perturb=False):
        ...
        outputs = self.model.render(rays_o, rays_d, poses, H, W, staged=True, perturb=perturb, light_d=light_d, ambient_ratio=ambient_ratio, shading=shading, bg_color=bg_color)
    ...
    def test(self, loader, save_path=None, name=None, write_video=True):
        ...
        if write_video:
            ...
            print('save video...', os.path.join(save_path, f'{name}_rgb.mp4'), os.path.join(save_path, f'{name}_depth.mp4'))
    ...
    def train_one_epoch(self, loader, max_epochs):
        ...
        for data in loader:
            ...
            with torch.cuda.amp.autocast(enabled=self.fp16):
                if self.opt.save_guidance and (self.global_step % self.opt.save_guidance_interval == 0 or self.global_step == 1):
        ...
        with torch.no_grad():
            ...
            for data in loader:
                ...
                if self.local_rank == 0:
                    ...
                    # save image
                    save_path = os.path.join(self.workspace, 'validation', f'{name}_rgb_{self.local_step:04d}.png')
                    save_path_depth = os.path.join(self.workspace, 'validation', f'{name}_depth_{self.local_step:04d}.png')
