...
from nerf.provider_3DPortraitGAN import NeRFDataset
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trigrid_lr_ratio', type=float, nargs='+', default=[100,100,100,100,100,100,100,40], help="stable diffusion time steps range")
    parser.add_argument('--scheduler_annealing', action='store_true', help="annealing scheduler")
    parser.add_argument('--use_body_pose', action='store_true', help="use_body_pose")
    ...
    # model options
    parser.add_argument('--bg_radius', type=float, default=3.0, help="if positive, use a background model at sphere(bg_radius)")
    ...
    parser.add_argument('--radius_range', type=float, nargs='*', default=[2.7, 2.7],
    ...
    parser.add_argument('--fovy_range', type=float, nargs='*', default=[12.447863, 12.447864],
                        help="training camera fovy range")
    ...
    parser.add_argument('--default_fovy', type=float, default=12.447863, help="fovy for the default view")
    ...
    if opt.learnable_bg:
        assert opt.bg_radius> max(opt.radius_range), f"bg_radius must be larger than max(radius_range) = {max(opt.radius_range)}"
    ...
    from nerf.network_trigrid_heirarchy import NeRFNetwork

    # load plane from pkl
    if os.path.isfile(opt.trigrid_path):
        print(f'loading plane from {opt.trigrid_path}...')
        import pickle
        ...
        with open(opt.trigrid_path, 'rb') as f:
            data = pickle.load(f)
            ws = data['ws']
            threeDRepresentation['ws'] = ws
            threeDRepresentation_shapes['ws'] = ws.shape
            for key in data:
                if 'trigrids' in key:
                    threeDRepresentation[key] = data[key]#.view(1, 3, -1, data[key].shape[-2], data[key].shape[-1])

                    threeDRepresentation_shapes[key] =threeDRepresentation[key].shape
    ...
    model = NeRFNetwork(
        opt=opt,
        device=device,
        trigrid_shapes=threeDRepresentation_shapes
    )
    ...
    if os.path.isfile(opt.trigrid_path):
        model.load_state_dict(
            threeDRepresentation, strict=False
        )
    ...
    if opt.six_views:
        raise NotImplementedError(f'--six_views {opt.six_views} is not implemented!')
        ...
        raise NotImplementedError(f'--test {opt.test} is not implemented!')
        guidance = None # no need to load guidance model at test
        ...
        if opt.optim == 'adan':
            ...
            optimizer = lambda model: Adan(model.get_params(5 * opt.lr,trigrid_lr_ratio = opt.trigrid_lr_ratio), eps=1e-8, weight_decay=2e-5, max_grad_norm=5.0,
                                           foreach=False)
        else:  # adam
            optimizer = lambda model: torch.optim.Adam(model.get_params(opt.lr,trigrid_lr_ratio = opt.trigrid_lr_ratio), betas=(0.9, 0.99), eps=1e-15)

        if opt.scheduler_annealing:
            print('===============  use scheduler: annealing')
        ...
        trainer = Trainer(' '.join(sys.argv), 'df', opt, model, guidance, device=device, workspace=opt.workspace,
                          optimizer=optimizer, ema_decay=0.95, fp16=opt.fp16, lr_scheduler=scheduler, use_checkpoint=opt.ckpt, scheduler_update_every_step=True)
        ...
        if opt.gui:
            ...
            # # test output
            # trainer.test(test_loader,save_path = os.path.join(opt.workspace, 'initiation'))
