...
@click.option('--cbase',        help='Capacity multiplier', metavar='INT',                      type=click.IntRange(min=1), default=18432, show_default=True)
@click.option('--cmax',         help='Max. feature maps', metavar='INT',                        type=click.IntRange(min=1), default=144, show_default=True)
...
@click.option('--radius_scale',       help='radius scale ratio.',  metavar='FLOAT',            type=click.FloatRange(min=0.0), default=0.7)
...
def main(**kwargs):
    ...
    if opts.cfg == 'full-head':
        rendering_options.update({
            'depth_resolution': 48,  # number of uniform samples to take per ray.
            'depth_resolution_importance': 48,  # number of importance samples to take per ray.
            # 'ray_start': 2.25 * opts.radius_scale,  # near point along each ray to start taking samples.
            # 'ray_end': 3.3 * opts.radius_scale,  # far point along each ray to stop taking samples.
            'ray_start': 2.25 + (2.7-2.25) * (1- opts.radius_scale),  # near point along each ray to start taking samples.
            'ray_end': (3.3-2.7) * opts.radius_scale + 2.7 ,  # far point along each ray to stop taking samples.
            'box_warp': 1* opts.radius_scale,
