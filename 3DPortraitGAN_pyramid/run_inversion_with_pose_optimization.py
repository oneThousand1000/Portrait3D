...
import dnnlib
import legacy
from proj.projector import w_projector_with_pose_optim
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_pkl', type=str,default='./models/model.pkl')
    parser.add_argument('--pose_prediction_kwargs_path', type=str,default='./models/model.json')
    parser.add_argument('--inversion_name', type=str)
    parser.add_argument('--with_pose_optim', action='store_true')
    parser.add_argument('--test_data_dir', type=str, default='../test_data')

    opt = parser.parse_args()
    model_pkl = opt.model_pkl
    pose_prediction_kwargs_path = opt.pose_prediction_kwargs_path
    inversion_name = opt.inversion_name
    with_pose_optim = opt.with_pose_optim
    ...
    # teaser
    todo = glob.glob(os.path.join(test_data_dir, '*'))
    ...
    for sub_dir in todo:
        ...
        res_dir = os.path.join(sub_dir, f'samples_new_crop/{inversion_name}')
        if os.path.exists(new_crop_samples_dir) and len(
            glob.glob(os.path.join(new_crop_samples_dir, f'{inversion_name}/*/inversion.pt'))) == 0:
            input_dir = new_crop_samples_dir
        ...
        # ----------------------------------------------------------------------------
            ...
            for image_path in images[:1]:
                ...
                if os.path.exists(f'{embedding_dir}/0.pt'):
                    ...
                    # id_image = torch.squeeze((image.to(global_config.device) + 1) / 2) * 255
                    w_pivot, p_opt = w_projector_with_pose_optim.project(G, c, p, embedding_dir, id_image,
                                                                         device=torch.device('cuda'),
                                                                         w_avg_samples=600,
                                                                         num_steps=500,
                                                                         w_name=image_name, no_sr=False)
                    # w_pivot = w_pivot.detach().clone().to(global_config.device)
                    w_pivot = w_pivot.to(global_config.device)
                    torch.save(w_pivot, f'{embedding_dir}/inversion.pt')

                    print('p_opt:', p_opt.shape)

                    poses = {
                        'pose': p_opt.detach().cpu().numpy().tolist(),
                        'camera_pose': c.cpu().numpy().tolist()
                    }
                    with open(f'{embedding_dir}/pose.json', 'w') as f:
                        json.dump(poses, f)
