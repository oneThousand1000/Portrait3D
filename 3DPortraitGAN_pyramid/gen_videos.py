...
@click.option('--large_pose', type=bool, help='Gen shapes for shape interpolation', default=False, show_default=True)
...
def generate_images(
    ...
    if interpolate:
        ...
        for seed in seeds:
            if large_pose:
                output = os.path.join(outdir, f'{seed}_large_pose.mp4')
            else:
                output = os.path.join(outdir, f'{seed}_small_pose.mp4')
            seeds_ = [seed]
            gen_interp_video(G=G, mp4=output, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes, w_frames=w_frames, seeds=seeds_, shuffle_seed=shuffle_seed, psi=truncation_psi, truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode,large_pose = large_pose)
