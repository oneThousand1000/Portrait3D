...
"""  
3D-aware stylegan2 backbone architectures from the paper
"Mimic3D: Thriving 3D-Aware GANs via 3D-to-2D Imitation (ICCV 2023)" 
https://github.com/SeanChenxy/Mimic3D/blob/main/training/networks_stylegan2.py
"""
...
class SynthesisLayer(torch.nn.Module):
    def __init__(self,
        ...
        if self.roll_out in ['b', 'a']:
            affine_scale = 9
        elif self.roll_out in ['s']:
            affine_scale = 3
        self.affine = FullyConnectedLayer(w_dim, in_channels * affine_scale, bias_init=1)
        memory_format = torch.channels_last if channels_last else torch.contiguous_format
        self.weight = torch.nn.Parameter(torch.randn(
            [out_channels, in_channels * (1, 3)[self.roll_out in ['b', 'a']],
             kernel_size, kernel_size]).to(memory_format=memory_format))
        if use_noise:
            self.register_buffer('noise_const', torch.randn([resolution, resolution * (1, 3)[self.roll_out == 'w']]))
            self.noise_strength = torch.nn.Parameter(torch.zeros([]))
        self.bias = torch.nn.Parameter(torch.zeros([out_channels]))

    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1, **_):
        assert noise_mode in ['random', 'const', 'none']
        # noise_mode = 'const'
        in_resolution = self.resolution // self.up
        misc.assert_shape(x, [None, self.in_channels, in_resolution, in_resolution * (1, 3)[self.roll_out == 'w']])
        styles = self.affine(w)
        if self.roll_out in ['b', 'a', 's']:
            styles = styles.view(styles.shape[0], 3, styles.shape[1] // 3).view(styles.shape[0] * 3,
                                                                                styles.shape[1] // 3)
        if self.roll_out in ['b', 'a', ]:
            x = aware3d_att(x) if self.roll_out == 'a' else aware3d(x)
        noise = None
        if self.use_noise and noise_mode == 'random':
            noise = torch.randn([x.shape[0], 1, self.resolution, self.resolution * (1, 3)[self.roll_out == 'w']],
...
def aware3d_att(x):
    x_ = x.view(-1, 3, x.shape[1], x.shape[2], x.shape[3])
    x_cyx, x_czy, x_cxz = x_[:, 0], x_[:, 1], x_[:, 2]

    x_yxc = x_cyx.permute(0, 2, 3, 1)
    x_ycz = x_czy.permute(0, 3, 1, 2)
    x_yzc = x_czy.permute(0, 3, 2, 1)
    x_yxz = torch.einsum('byxc,bycz->byxz', x_yxc, x_ycz)
    x_yxz = torch.softmax(x_yxz, dim=-1)
    x_cyx_f_czy = torch.einsum('byxz,byzc->byxc', x_yxz, x_yzc).permute(0, 3, 1, 2)
    x_xyc = x_cyx.permute(0, 3, 2, 1)
    x_xcz = x_cxz.permute(0, 2, 1, 3)
    x_xzc = x_cxz.permute(0, 2, 3, 1)
    x_xyz = torch.einsum('bxyc,bxcz->bxyz', x_xyc, x_xcz)
...
class SynthesisBlock(torch.nn.Module):
    def __init__(self,
        ...
        if in_channels == 0:
            self.const = torch.nn.Parameter(torch.randn([(1, 3)[self.roll_out in ['b', 'a']], out_channels, resolution,
                                                         resolution * (1, 3)[self.roll_out == 'w']]))

        if in_channels != 0:
            self.conv0 = SynthesisLayer(in_channels, out_channels, w_dim=w_dim, resolution=resolution, up=self.up,
                                        roll_out=roll_out,
                                        resample_filter=resample_filter, conv_clamp=conv_clamp,
                                        channels_last=self.channels_last, **layer_kwargs)
            self.num_conv += 1

        self.conv1 = SynthesisLayer(out_channels, out_channels, w_dim=w_dim, resolution=resolution, roll_out=roll_out,
                                    conv_clamp=conv_clamp, channels_last=self.channels_last, **layer_kwargs)
        self.num_conv += 1

        if is_last or architecture == 'skip':
            self.torgb = ToRGBLayer(out_channels, img_channels, w_dim=w_dim,
                                    conv_clamp=conv_clamp, channels_last=self.channels_last, roll_out=self.roll_out)
            self.num_torgb += 1
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None, update_emas=False, **layer_kwargs):
        ...
        if self.in_channels == 0:
            ...
            x = x.repeat([ws.shape[0], 1, 1, 1])
        else:
            misc.assert_shape(x, [None, self.in_channels, self.resolution // self.up,
                                  self.resolution // self.up * (1, 3)[self.roll_out == 'w']])
        ...
        if img is not None and self.up > 1:
            misc.assert_shape(img, [None, self.img_channels, self.resolution // self.up,
                                    self.resolution // self.up * (1, 3)[self.roll_out == 'w']])
...
@persistence.persistent_class
class Hierarchy3DAwareSynthesisNetwork(torch.nn.Module):
    def __init__(self,
                 w_dim,  # Intermediate latent (W) dimensionality.
        ...
        for res in self.block_resolutions:
            in_channels = img_channels if res > 4 else 0
            out_channels = img_channels
            use_fp16 = (res >= fp16_resolution)
            is_last = (res == self.img_resolution) and self.add_block == 0
            ...
            if res in aware3d_res:
                block3d = Aware3DBlock(img_channels, res, w_dim, aware3d_att,
                                       block_kwargs.copy())
                setattr(self, f'b3d{res}', block3d)
    ...
    def forward(self, ws, **block_kwargs):
        ...
        x = img = img3d = None
        feature_maps = {}
        last_has_block3d = False
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            block = getattr(self, f'b{res}')
            block3d = getattr(self, f'b3d{res}', None)
            if last_has_block3d and block3d is None:
                assert NotImplementedError
                img = img3d.view(-1, 3, img3d.shape[-3], img.shape[-2], img.shape[-1]).view(img.shape)
            # 2D Branch
            x, img = block(x, img, cur_ws, **block_kwargs) # 2D Synthesis Block

            # 3D Branch
            if block3d is not None:
                last_has_block3d = True
                img3d = block3d(img3d, img, cur_ws, block_kwargs) # 3D-Aware Block
                if isinstance(img3d, list):
                    assert NotImplementedError
                else:
                    feature_maps[res] = img3d
            else:
                assert NotImplementedError
...
@persistence.persistent_class
class SR3DBlock(torch.nn.Module):
    def __init__(self, img_channels, img_resolution, w_dim, block_kwargs):
        super().__init__()
        block_kwargs['roll_out'] = 's'
        self.block2 = SynthesisBlock(img_channels // 3, img_channels // 3, w_dim=w_dim, resolution=img_resolution * 2,
                                     up=2,
                                     img_channels=32, is_last=True, use_fp16=False, **block_kwargs)
        self.block3 = SynthesisBlock(img_channels // 3, img_channels // 3, w_dim=w_dim, resolution=img_resolution * 2,
                                     up=1,
                                     img_channels=32, is_last=True, use_fp16=False, **block_kwargs)

    def forward(self, img, ws):
        ws = ws[:, -1:, :].repeat(1, 3, 1)
        img = img.view(img.shape[0], 3, -1, img.shape[-2], img.shape[-1]).view(img.shape[0] * 3, -1, img.shape[-2],
                                                                               img.shape[-1])
        x, img2 = self.block2(img, None, ws)
        x, img3 = self.block3(img2, None, ws)
        img2 = img2.view(-1, 3, img2.shape[-3], img2.shape[-2], img2.shape[-1]).view(-1, 3 * img2.shape[-3],
                                                                                     img2.shape[-2], img2.shape[-1])
        img3 = img3.view(-1, 3, img3.shape[-3], img3.shape[-2], img.shape[-1]).view(-1, 3 * img3.shape[-3],
                                                                                    img3.shape[-2], img3.shape[-1])
...
# ----------------------------------------------------------------------------
@persistence.persistent_class
class Aware3DBlock(torch.nn.Module):

    def __init__(self, img_channels, img_resolution, w_dim, aware3d_att, block_kwargs):
        super().__init__()
        block_kwargs['roll_out'] = ('b', 'a')[aware3d_att]
        up = 2
        self.block = SynthesisBlock(img_channels // 3, img_channels // 3, w_dim=w_dim, resolution=img_resolution * up,
                                    up=up,
                                    img_channels=img_channels // 3, is_last=True, use_fp16=False, **block_kwargs)
    ...
    def forward(self, x, img, ws, block_kwargs):
        ...
        ws = ws[:, -1:, :].repeat(1, 3, 1)
        _, img = self.block(img, None, ws, **block_kwargs)
        return img


@persistence.persistent_class
class Hierarchy3DAwareGenerator(torch.nn.Module):
    def __init__(self,
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = Hierarchy3DAwareSynthesisNetwork(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels,
                                          **synthesis_kwargs)
