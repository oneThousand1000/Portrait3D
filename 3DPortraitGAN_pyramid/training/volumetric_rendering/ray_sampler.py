...
class RaySampler(torch.nn.Module):
    ...
    def patch_forward(self, cam2world_matrix, intrinsics, resolution, patch_scale=1):
        """
        Create batches of rays and return origins and directions.
        ...
        patch_info = []
        uv = torch.stack(
            torch.meshgrid(torch.arange(full_resolution, dtype=torch.float32, device=cam2world_matrix.device),
                           torch.arange(full_resolution, dtype=torch.float32, device=cam2world_matrix.device),
                           indexing='ij')) * (1. / full_resolution) + (0.5 / full_resolution)
