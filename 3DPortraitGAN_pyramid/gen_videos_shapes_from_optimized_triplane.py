...
def gen_interp_video(G, mp4: str, trigrid=None,ws=None, shuffle_seed=None, w_frames=60*4, kind='cubic',
        ...
        for yi in range(grid_h):
            for xi in range(grid_w):
                ...
                if large_pose and frame_idx % int(num_keyframes * w_frames//8) == 0:
                    image_row_8.append((img.permute(1, 2, 0) * 127.5 + 128).clamp(0, 255).to(torch.uint8))

                if large_pose and frame_idx % int(num_keyframes * w_frames//4) == 0:
                    image_row_4.append((img.permute(1, 2, 0) * 127.5 + 128).clamp(0, 255).to(torch.uint8))
    ...
    if large_pose:
        import PIL
        image_row_8 = torch.cat(image_row_8, 1).cpu().numpy()
        PIL.Image.fromarray(image_row_8.astype(np.uint8)).save(mp4.replace('.mp4', '_final_8.png'))

        image_row_4 = torch.cat(image_row_4, 1).cpu().numpy()
        PIL.Image.fromarray(image_row_4.astype(np.uint8)).save(mp4.replace('.mp4', '_final_4.png'))
...
def generate_images(
    ...
    outdir = os.path.join(res_dir, 'results_final')
    os.makedirs(outdir, exist_ok=True)
    if not os.path.exists(os.path.join(res_dir, 'log/ckpt')):
        print('WARNING: No checkpoints found in "%s"!' % os.path.join(res_dir, 'log/ckpt'))
        return
    ...
    ckpt_path = glob.glob(os.path.join(res_dir, 'log/ckpt/*'))
    ckpt_path = sorted(ckpt_path)
    ckpt_path = ckpt_path[-1]
    if not os.path.exists(ckpt_path):
        print('WARNING: No checkpoints found in "%s"!' % ckpt_path)
        return
    ...
    output = os.path.join(outdir, f'large_pose_final.mp4')
    print('Generating video "%s"...' % output)
    if not os.path.exists(output):
        gen_interp_video(G=G, mp4=output, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes, w_frames=240,
    ...
    output = os.path.join(outdir, f'small_pose_final.mp4')
    print('Generating video "%s"...' % output)
    if not os.path.exists(output):
        gen_interp_video(G=G, mp4=output, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes, w_frames=120,
                         trigrid=trigrid, ws=ws, shuffle_seed=shuffle_seed, psi=truncation_psi,
